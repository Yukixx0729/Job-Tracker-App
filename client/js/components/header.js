  ...
  if (!dueDataJob.rowCount && !dueDataTodo.length) {
    ...
    for (let due of filteredData) {
      ...
      if (Object.keys(due)[2] === "description") {
        dueItem.innerHTML = `
        <p>Task: ${due.title}</p> <p> Priority : ${due.priority}</p>
        <p>Due date: ${due.due_date.substring(0, 10)}</p> <p>Status: ${due.status}</p>
        `
      ...
      if (Object.keys(due)[2] === "company") {
        if (!due.stages) {
          dueItem.innerHTML = `
            <p>Job company: ${due.company}</p> 
            <p>Job title: ${due.title}</p> 
            <p>Due date: ${due.due_date.substring(0, 10)}</p>
            `
        } else {
          dueItem.innerHTML = `
            <p>Job company: ${due.company}</p> 
            <p>Job title: ${due.title}</p> 
            <p>Due date: ${due.due_date.substring(0, 10)}</p><p>Stage: ${due.stages}</p>
           `
        }
