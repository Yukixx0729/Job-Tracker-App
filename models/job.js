...
const getAllJobs = (userId) => {
  return db.query("SELECT * FROM jobs WHERE user_id =$1;", [userId])
    .then((result) => result)
}
...
const getJobByUserId = (user_id) => {
  return db.query("SELECT * FROM jobs WHERE user_id=$1", [user_id])
    .then((result) => result);
}

const addJob = ( title, company, location, description, job_url, due_date, stages, user_id ) => {
  const sql = `INSERT INTO jobs (title, company, location, description, job_url, due_date, stages,user_id) VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING id;`
  return db.query(sql, [ title, company, location, description, job_url, due_date, stages, user_id ])
}
